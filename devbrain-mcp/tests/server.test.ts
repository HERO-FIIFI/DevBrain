...
describe('MCP contracts', () => it('registers nineteen tools and returns validated structured content', async () => {
  ...
  const listed = await client.listTools(); expect(listed.tools.map((tool) => tool.name).sort()).toEqual(['devbrain_context_pack','devbrain_dependency_audit','devbrain_diagnose_log','devbrain_find_references','devbrain_find_todos','devbrain_get_diff','devbrain_get_execution_evidence','devbrain_git_history','devbrain_inspect_docker','devbrain_project_health','devbrain_read_file_slice','devbrain_read_symbol','devbrain_repo_map','devbrain_run_build','devbrain_run_lint','devbrain_run_targeted_tests','devbrain_run_tests','devbrain_search_symbols','devbrain_search_text']);

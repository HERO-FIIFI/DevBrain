...
const summarySchema=z.object({framework:z.string(),passed:z.number().int().optional(),failed:z.number().int().optional(),skipped:z.number().int().optional(),errors:z.number().int().optional(),failingTests:z.array(z.object({name:z.string(),message:z.string()})),failingTestsTotal:z.number().int(),failingTestsReturned:z.number().int(),failingTestsTruncated:z.boolean(),parseMethod:z.enum(['text_regex','exit_code_only','unknown']),parseConfidence:z.enum(['medium','low','unknown'])});
